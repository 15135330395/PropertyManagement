...
<div class="x-body">
    ...
    <script type="text/javascript" id="edate">
        {{ dateFormat(d.edate) }}
    </script>
</div>
</body>
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#tab'
            // Servlet 返回一个json字符串
            ,url:'<%=request.getContextPath()%>/EqumaintenanceServlet?action=queryPage'
            ,toolbar: '#toolbarDemo'
            ,title: '合同列表'
            ,cols: [[
                {type: 'checkbox', fixed: 'left'}
                ,{field:'eid', title:'ID', width:'10%', fixed: 'left', unresize: true, sort: true}
                ,{field:'ename', title:'设备名称', width:'10%'}
                ,{field:'eno', title:'设备编号', width:'10%'}
                ,{field:'content', title:'保养过程', width:'10%'}
                ,{field:'charge', title:'负责人', width:'10%'}
                ,{field:'edate', title:'保养日期', width:'10%' ,templet:'#edate'  }
                ,{field:'material', title:'领取物料名称', width:'10%'}
                ,{field:'number', title:'领取物料数量', width:'10%'}
            ...
            switch(obj.event){
                ...
                case 'toadd':
                    ...
                    layer.open({
                        ...
                        offset: 'auto', //右下角弹出
                        anim: 2,
                        content: ['equmaintenanceAdd.jsp'], //iframe的url，no代表不显示滚动条 , 'no'
            ...
            if(obj.event === 'del'){
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        type:"post",
                        url:"<%=request.getContextPath()%>/EqumaintenanceServlet",
                        data:"action=delete&eid="+data.eid,
                        success:function(msg){
                            obj.del();
                            if(msg==0){
                ...
                layer.open({
                    ...
                    offset: 'auto', //右下角弹出
                    anim: 2,
                    content: ['<%=request.getContextPath()%>/EqumaintenanceServlet?action=queryOneBack&eid='+data.eid] //iframe的url，no代表不显示滚动条
