...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#tab'
            // Servlet 返回一个json字符串
            ,url:'<%=request.getContextPath()%>/EqumalfunctionServlet?action=queryPage'
            ,toolbar: '#toolbarDemo'
            ,title: '合同列表'
            ,cols: [[
                ...
                ,{field:'maintenance', title:'保养情况', width:'10%'}
                ,{field:'rejection', title:'报废', width:'10%'}
            ...
            switch(obj.event){
                ...
                case 'toadd':
                    ...
                    layer.open({
                        ...
                        offset: 'auto', //右下角弹出
                        anim: 2,
                        content: ['equmalfunctionAdd.jsp'], //iframe的url，no代表不显示滚动条 , 'no'
            ...
            if(obj.event === 'del'){
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        type:"post",
                        url:"<%=request.getContextPath()%>/EqumalfunctionServlet",
                        data:"action=delete&mid="+data.mid,
                        success:function(msg){
                            obj.del();
                            if(msg==0){
                ...
                layer.open({
                    ...
                    offset: 'auto', //右下角弹出
                    anim: 2,
                    content: ['<%=request.getContextPath()%>/EqumalfunctionServlet?action=queryOneBack&mid='+data.mid] //iframe的url，no代表不显示滚动条
