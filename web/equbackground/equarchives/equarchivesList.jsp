...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#tab'
            // Servlet 返回一个json字符串
            ,url:'<%=request.getContextPath()%>/EquarchivesServlet?action=queryPage'
            ,toolbar: '#toolbarDemo'
            ,title: '合同列表'
            ,cols: [[
                {type: 'checkbox', fixed: 'left'}
                ,{field:'id', title:'ID', width:'5%', fixed: 'left', unresize: true, sort: true}
                ,{field:'ano', title:'档案编号', width:'10%'}
            ...
            switch(obj.event){
                ...
                case 'toadd':
                    ...
                    layer.open({
                        ...
                        offset: 'auto', //右下角弹出
                        anim: 2,
                        content: ['equarchivesAdd.jsp'], //iframe的url，no代表不显示滚动条 , 'no'
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.open({
                    ...
                    offset: 'auto', //右下角弹出
                    anim: 2,
                    content: ['<%=request.getContextPath()%>/EquarchivesServlet?action=queryOneBack&id='+data.id] //iframe的url，no代表不显示滚动条
