...
<div class="x-body">
    <xblock>
        ...
        <button name="name" id="name" class="layui-btn"
                onclick="x_admin_show('添加客户信息','<%=request.getContextPath()%>/AddrServlet')">
    ...
    <table class="layui-table" id="tab">
        ...
        <c:forEach items="${customerList}" var="cList">
            <tr>
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${cList.id}'><i
                            class="layui-icon">&#xe605;</i></div>
                </td>
                <td>${cList.id}</td>
                <td>${cList.name}</td>
    ...
    <div class="page">
        <div>
            <a class="prev" href="<%=request.getContextPath()%>/CustomerServlet?action=query&pageIndex=${pageBean.pages}">&lt;&lt;</a>
            <c:forEach var="i" begin="1" end="${pageBean.pages}" step="1">
                <c:if test="${i==pageBean.pageIndex}">
                    <span class="current">${i}</span>
                </c:if>
                <c:if test="${i!=pageBean.pageIndex}">
                    <a class="num" href="<%=request.getContextPath()%>/CustomerServlet?action=query&pageIndex=${i}">${i}</a>
                </c:if>
            </c:forEach>
            <a class="next" href="<%=request.getContextPath()%>/CustomerServlet?action=query&pageIndex=${pageBean.pages}">&gt;&gt;</a>
        </div>
    </div>
