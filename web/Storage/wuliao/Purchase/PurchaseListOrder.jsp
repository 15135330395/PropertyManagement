...
<div class="x-body">
    ...
    <script>
        ...
        function query(){
            var start = $("#start").val();
            var end=$("#end").val();
            location.href="<%=request.getContextPath()%>/PurchaseServlet?action=queryLimitDate&startDate="+start+"&endDate="+end
        }
        ...
        layui.use('table', function(){
            ...
            table.render({
                elem: '#tab'
                //返回一个json字符串
                ,url:'<%=request.getContextPath()%>/PurchaseServlet?action=PurchaseListOrderFindAll'
                ,toolbar: '#toolbarDemao'
                ,title: '采购订单明细查询'
                ,cols: [[
                    {type: 'checkbox', fixed: 'left'}
                    ,{field:'id', title:'仓库ID', width:'15%', fixed: 'left', sort: true}
                    ,{field:'goodName', title:'物品名称', width:'20%',}
                    ,{field:'numbers', title:'数量', width:'13%', sort: true}
                    ,{field:'cost', title:'总计（元）', width:'15%', sort: true}
                    ,{field:'date', title:'日期', width:'15%', sort: true}
                    ,{field:'listNumber', title:'订单号', width:'18%',}
                ]]
                ,page: true
            });
