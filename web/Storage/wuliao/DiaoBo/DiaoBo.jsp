...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so layui-form-pane">
            <input class="layui-input" value="" placeholder="开始日" name="start" id="start">
            <input class="layui-input" value="" placeholder="截止日" name="end" id="end">
            <a class="layui-btn layui-btn-xs" onclick="query()">查询</a>
        </form>
    </div>
    <div class="x-body">
        <a class="layui-btn layui-btn-xs" onclick="add()">调拨</a>
    ...
    <script>
        ...
        function query(){
            var start = $("#start").val();
            var end=$("#end").val();
            location.href="<%=request.getContextPath()%>/DiaoBoServlet?action=queryLimitDate&startDate="+start+"&endDate="+end
        }
        ...
        layui.use('table', function(){
            ...
            table.render({
                elem: '#tab'
                //返回一个json字符串
                ,url:'<%=request.getContextPath()%>/DiaoBoServlet?action=findAll'
                ,toolbar: '#toolbarDemo'
                ,title: '调拨记录'
                ,cols: [[
                    {type: 'checkbox', fixed: 'left'}
                    ,{field:'outStorage', title:'出库仓库ID', width:'15%', fixed: 'left', sort: true}
                    ,{field:'inStorage', title:'入库仓库ID', width:'15%',sort: true}
                    ,{field:'goodName', title:'物品名称', width:'20%',}
                    ,{field:'numbers', title:'数量', width:'21%', sort: true}
