...
public class MenuDaoImpl implements MenuDao {
    ...
    public List<Menu> findMenuByUserId(int userId) {
        String sql = "select * from xtgl_menu where menu_id in ( " +
                "select menu_id from xtgl_role_menu where role_id =( " +
                " select role_id from xtgl_role where role_id = (" +
                " select role_id from xtgl_user_role where user_id =? )))";
        try {
            List<Menu> list = queryRunner.query(sql, new BeanListHandler<>(Menu.class), userId);
            return list;
        } catch (SQLException e) {
            e.printStackTrace();
        }
