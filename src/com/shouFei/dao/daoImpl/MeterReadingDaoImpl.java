...
public class MeterReadingDaoImpl implements MeterReadingDao {
    final static String colname="meter_reading_id meterReadingId, plot_name plotName, room_number roomNumber, riqi, price, start, stop, pooled";
    ...
    public int updateMeterReading(MeterReading meterReading) {
        ...
        try {
            int i = queryRunner.update(JdbcUtil.getConnection(), sql, meterReading.getPlotName(),meterReading.getRoomNumber()
    ...
    public MeterReading findMeterReadingById(int id) {
       ...
       // String sql="select * from meter_reading where meter_reading_id=?";
        ...
        ResultSet rs = null;
        try {
            //queryRunner.query(JdbcUtil.getConnection(),sql,new BeanListHandler<MeterReading>(MeterReading.class),id);
            Connection connection = JdbcUtil.getConnection();
            ps = connection.prepareStatement(sql);
            ps.setInt(1,id);
            rs = ps.executeQuery();
